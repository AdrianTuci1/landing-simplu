...
import { DataModelDiagramMobile } from "./data-model-diagram-mobile";
...
export const EntityCard = ({ entity, className }) => {
    ...
    <div className={`group relative w-[141px] lg:w-[259px] rounded-[10px] p-[7px] border lg:rounded-xl lg:p-[11px] border-weak-stroke bg-white-100 shadow-[0px_2px_3px_-2px_rgba(28,40,64,0.10),0px_4px_6px_-2px_rgba(28,40,64,0.04)] ${className}`} style={{ borderColor: 'var(--internal-color-weak-stroke, #e5e7eb)' }}>
      <div className="pointer-events-none select-none">
        <div className="flex items-center justify-between">
          ...
          <div className={`border font-medium rounded-md px-[3px] py-[0.5px] text-[8px] leading-[11px] lg:rounded-lg lg:px-[5px] lg:py-px lg:text-[12px] lg:leading-4 ${isCustom
...
const DataModelDiagram = () => {
  ...
  return (
    <section className="relative flex h-full w-full items-stretch flex-col pt-10 pb-10 px-4 sm:px-6 lg:px-8">
      {/* Menu Selection */}
      <div className="flex flex-col sm:flex-row gap-2 justify-center my-8 px-4">
        <button
          onClick={() => setActiveMenu("clinici-dentare")}
          className={`px-4 py-2 text-sm rounded-lg border transition-all ${activeMenu === "clinici-dentare"
        ...
        <button
          ...
          className={`px-4 py-2 text-sm rounded-lg border transition-all ${activeMenu === "hoteluri"
            ? "bg-gray-100 border-gray-300 text-gray-900 font-medium"
            : "bg-white border-border text-foreground hover:bg-gray-50"
            }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 text-sm rounded-lg border transition-all ${activeMenu === "sali-fitness"
            ? "bg-gray-100 border-gray-300 text-gray-900 font-medium"
            : "bg-white border-border text-foreground hover:bg-gray-50"
            }`}
        >
      ...
      {/* Mobile Diagram */}
      <DataModelDiagramMobile entities={entities} />

      {/* Desktop Diagram Container */}
      <div className="relative mx-0 sm:mx-5 lg:mx-20 hidden lg:grid mb-12 overflow-x-auto grid-cols-[auto_2fr_auto_3fr_auto_2fr_auto] px-4 sm:px-0">

        {/* Entity 1 (Slot 1 - Top Left) */}
        <div className="opacity-1 filter-blur-0 transform-none lg:mt-[50px]">
                  ...
                  {Object.values(row).map((cell, cellIdx) => (
                    <td key={cellIdx} className="px-4 py-3 text-sm text-foreground">
                      {cellIdx === Object.values(row).length - 1 ? (
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-1 rounded text-xs ${cell === "Confirmată"
